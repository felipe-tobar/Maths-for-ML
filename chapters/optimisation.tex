...
As discussed previously, achieving good generalisation does not simply require arbitrarily wide networks. The \textbf{architecture} of a neural network refers to its overall structure: number of layers, neurons per layer, connectivity, and activation functions.
...
\begin{equation}
	\hat{y} = g(h^{(l)}U + c),
\end{equation}
where $g$ denotes the output activation defining the \textbf{output unit}.
...
The \textbf{backpropagation} algorithm propagates cost information backwards to compute gradients efficiently. While analytic gradients can be derived, automatic differentiation provides faster and more scalable computation, enabling modern deep learning.

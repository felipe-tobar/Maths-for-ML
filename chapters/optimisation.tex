...
\begin{figure}[ht]
    ...
    \caption{Example of a linear regression model minimising the least squares loss.}
...
\end{align}
Figure \ref{fig:log_reg_example} shows an example of a binary classification task minimising the cross entropy to separate data generated from two Gaussian distributions.
...
\begin{figure}[ht]
    ...
    \caption{Implementation of the $K$-means method to clustering data from two Gaussians. The figure shows the learnt centroids (black cross) and the colours correspond to the cluster assignments.}
...
\begin{itemize}
	...
	\item \textbf{Optimisation variable:} Minimising $f$ requires finding the value of $x$, such that $f(x)$ is minimum. This is also written as
	\begin{equation}
	...
	\end{equation}
	\item \textbf{Local / global optima.} Values for the optimisation variable that solve the optimisation problem either locally or globally. More formally: 
	\begin{align}
...
In unconstrained optimisation, we have functions that have a unique global minimum and others that have more than one. For instance, Figure \ref{fig:example_local_global} shows $(x-1)^2 + (y+2)^2$ on the left, which has a unique minimiser on $(x, y) = (1, -2)$ and $(x^2-1)^2 + (y^2 -1)^2$ on the right, where every global minimiser belongs to the set $\{(x,y) : x=+-1 \land y=+-1\}$.
\end{mdframed}

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\textwidth]{img/optimisation_example_2d_parametric.pdf}
    \caption{Functions with a unique (left) and non-unique (right) global minima.}
    \label{fig:example_local_global}
\end{figure}


\begin{mdframed}[style=discusion, frametitle={\center Interplay between constraints and local/global optima}]
On the other hand, we may be presented with functions such as $f(x,y) = \sin(x)*\sin(y) + 0.1(x^2+y^2)$, which has a global minimum but also local minima. Figure \ref{fig:example_restricted_minima} shows how different restrictions change the number and type of optima. In this case, when restricting the minimisation problem to the circle centred in $(1.5,1)$ with radius $2$, we observe a minimiser that is not the global minimum of the unconstrained problem (an unfeasible point) nor one of the local minima.
...
\begin{figure}[ht]
    ...
    \caption{Minima for constrained (left) and unconstrained (right) problems.}
...
\begin{equation}
\exists\, \delta > 0  \;\; 
\text{s.t.} \;\; \forall\, \theta \in \Theta \;\; 
\|\theta - \theta_\star\| < \delta 
\;\Rightarrow\;
L(\theta_\star) \le L(\theta).
\end{equation}

We now review the \textbf{optimality conditions}.
...
\end{itemize}
Figure \ref{fig:critical_points} shows the function behaviour for different $p$. What happens when $|p|=1$?
...
\end{equation}
\end{definition}
For illustration, Figure \ref{fig:convex_nonconvex_sets} shows two convex and two non-convex sets.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\textwidth]{img/optimisation_convex_and_nonconvex_regions.pdf}
    \caption{Examples of convex and non-convex sets, highlighted in blue.}
...
In general, finding a minimum by setting $\nabla f(x) = 0$ and solving for $x$ is not possible. For that reason, we will consider iterative methods based on gradients. The idea here is to go \emph{downhill} following the gradient towards the minimum (ignoring the curvature information for now). 
...
The step size $\eta_t$ is also known as \emph{learning rate}. Furthermore,  we refer to the set $\{\eta_1,\eta_2,\ldots\}$ as the learning rate schedule. We will usually consider a constant learning rate, that is, $\eta_t = \eta$, $\forall t\in\N$. Though this is the simplest choice, there are some concerns to it: if $\eta$ is too large, the iteration may fail to converge; whereas if it is too small, it may not converge at all. 
...
As shown in Figure \ref{fig:newton_method}, this method converges in one step (for a quadratic function). That is, the update corresponds to the closed form solution of the minimisation problem.
...
\begin{equation}
	L(x) = \E_{q(z)}L(x,z),
\end{equation}
that is, when the loss function is random and we aim to minimise its expected value.  For instance, in linear regression we have $L(\theta)=\E_{q(y|x)}(y-\theta^\top x)$.
...
Figure \ref{fig:stochastic_loss} shows how the empirical loss changes as we consider more and more samples.
...
\end{equation}
where the \emph{forgetting factor} $\beta$ is usually chosen close to 0.9. Then, the update rule also follows
\begin{equation}
	\Delta \theta_t = -\eta_t \frac{1}{s_t} g_t.
\end{equation}
...
The de facto optimiser in deep learning is Adam (Adaptive Moment Estimation) proposed by \cite{kingma2015adam}, which combines RMSProp and momentum. Adam's update follows: 
\begin{align}
...
We visualise the effect of the different optimisers presented above in Figure \ref{fig:optimisers}. Notice that SGD converges more slowly than RMSprop and Adam. Adagrad, on the other hand, slows down too early.
...
%%%%%%%%%########
\subsection{Training and Regularisation of Neural Networks}

\subsubsection{Network Architecture}

The \textbf{architecture} of a neural network refers to its overall structure: number of layers, neurons per layer, connectivity, and activation functions.

\begin{figure}[H]
	\centering
	\includegraphics[scale=.5]{img/optimisation_nn.png}
	\caption{A neural network of depth $l$.}
	\label{fig:dnn}
\end{figure}

We adopt the following notation (as illustrated in Fig.~\ref{fig:dnn}):
\begin{equation}
	h^{(k)} = f^{(k)}(h^{(k-1)}W^{(k)} + b^{(k)}), \quad k \in \{1,\dots,l\}, \quad h^{(0)} = x,
\end{equation}
\begin{equation}
	\hat{y} = g(h^{(l)}U + c),
\end{equation}
where $f$ is the activation function for all (hidden) neurons, and $g$ is the activation function of the \textbf{output neurons}. Furthermore, $h^{(k)}$ denotes the output of the $k$-th layer. 

\subsubsection{Cost Function and Output Units}

Unlike linear models, the use of nonlinear activations renders the cost function generally non-convex, so gradient descent does not guarantee convergence to a global optimum. Although least squares may be used, in practice one minimises the negative log-likelihood of a probabilistic model $p(y|\bm{x};\bm{\theta})$:
\begin{equation}
	\label{eq:loss_NN_optim}
	J(\bm{\theta}) = -\E_{(\bm{x},\bm{y}) \sim \hat{p}_{\mathrm{data}}} [\log p_{\mathrm{model}}(\bm{y}|\bm{x})].
\end{equation}
The choice of output unit determines the appropriate likelihood and loss:
\begin{enumerate}
	\item \textbf{Linear:} $\hat{\bm{y}} = h^{(l)}U + c$  
	
	Models $\mathcal{N}(\bm{y}|\hat{\bm{y}}, \bm{I})$ and is equivalent to minimising MSE; suited for regression.
	
	\item \textbf{Sigmoid:} $\hat{y} = \mathrm{sig}(h^{(l)}U + c)$  
	
	Used for binary classification with $p(y|\bm{x}) = \mathrm{Bernoulli}(y|p)$ and $\hat{y} = p(y=1|\bm{x})$.
	
	\item \textbf{Softmax:} $\hat{\bm{y}} = \mathrm{softmax}(h^{(l)}U + c)$  
...
Recall that, in practice, since $p_{\mathrm{data}}$ in eq.~\eqref{eq:loss_NN_optim} is unknown, we optimise an empirical estimate of that loss given by 
\begin{equation}
	\label{eq:loss_NN_optim_emp}
	J(\bm{\theta}) = -\sum_i \log p_{\mathrm{model}}(\bm{y}_i|\bm{x}_i).
\end{equation}
The rest of this section is devoted to the calculation of this loss and its gradient. 

\subsubsection{Forward Propagation}

In a feedforward network, information flows from input $\bm{x}$ to output $\hat{\bm{y}}$. During training, this process computes the cost $J(\bm{X},\bm{\theta})$.

\begin{algorithm}[H]
	\caption{Forward Propagation}
	\textbf{Require:} Depth $l$, weights $\bm{W}^{(k)}$, biases $\bm{b}^{(k)}$, input $\bm{x}$, output $\bm{y}$, and output parameters $U$, $c$, $g$.\\
	\begin{algorithmic}[1]
...
\subsubsection{Backward Propagation – Preliminaries}

The \textbf{backpropagation} algorithm propagates cost information backwards to compute gradients efficiently. In practice, this is achieved by means of \textbf{automatic differentiation}, which provides fast and scalable computation, thus enabling modern deep learning.

Assume training is performed in \textbf{mini-batches} $(x^d)_{d=1}^N$, enabling efficient matrix operations on GPUs. For an MSE loss in regression:
\[
J(\bm{X},\bm{\theta}) = \frac{1}{2N}\sum_{d=1}^N (\hat{y}_d - y_d)^2,
\]
where $J_d=(\hat{y}_d - y_d)^2$, our goal is to update all weights $w_{ij}^{(k)}$. Using the chain rule:
\[
\frac{\partial J_d}{\partial w_{ij}^{(k)}} 
= \frac{\partial J_d}{\partial u_{dj}^{(k)}} 
  \frac{\partial u_{dj}^{(k)}}{\partial w_{ij}^{(k)}},
\]
where we define the \textbf{error term}
\[
...
\begin{equation}
\frac{\partial J}{\partial W^{(k)}} = (h^{(k-1)})^T @ \delta^{(k)}.
\end{equation}

\subsubsection{Backward Propagation – Hidden Layers}
...
\subsubsection{Backward Propagation – Output Layer}
...
Furthermore,
\begin{equation}
\frac{\partial J}{\partial b^{(k)}} = \mathrm{Sum}_1(\delta^{(k)}), \quad
\frac{\partial J}{\partial U} = (h^{(l)})^T @ \delta^{(l)}, \quad
\frac{\partial J}{\partial c} = \mathrm{Sum}_1(\delta^{(l)}),
\end{equation}
where $\mathrm{Sum}_1(A)$ denotes summation over the first axis.

\begin{algorithm}[H]
	\caption{Backward Propagation}
	\textbf{Require:} Learning rate $\lambda$.\\
	\begin{algorithmic}[1]
	\State Perform forward propagation, store $(\hat{y}), (u^{(k)}), (h^{(k)})$.
	\State Compute output error $\delta^{(l)}$ according to the output unit.
	\State Update $U \gets U - \lambda \frac{\partial J}{\partial U}$, \quad $c \gets c - \lambda \frac{\partial J}{\partial c}$.
	\For{$k = l,\dots,1$}
		\State Compute $\delta^{(k)}$ using the hidden-layer recursion.
		\State Evaluate $\frac{\partial J}{\partial W^{(k)}}, \frac{\partial J}{\partial b^{(k)}}$.
...
\begin{remark}
	The algorithm is applied to each mini-batch for several \textbf{epochs}. Increasing epochs generally decreases training error, but excessive training may degrade generalisation due to \textbf{overfitting}.
\end{remark}

\subsubsection{Regularisation}

Neural networks are often large models; controlling their complexity is crucial. \textbf{Regularisation} techniques aim to reduce the generalisation error while retaining sufficient model capacity.

\paragraph{L2 Regularisation.} Adds a quadratic penalty on weights:
\[
\tilde{J}(\bm{\theta}) = J(\bm{\theta}) + \frac{\alpha}{2}\|\bm{\theta}\|_2^2.
\]
The hyperparameter $\alpha$ controls the regularisation strength. Usually, only weights are penalised, not biases. In practice this corresponds to \textit{weight decay}:
\[

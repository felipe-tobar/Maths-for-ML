...
\begin{mdframed}[style=ejemplo, frametitle={\center Discrete RV: time to first heads}]
...
\begin{cases}
	...
	0, & \text{(Tails)},
\end{cases}
\quad\text{with } \Pb(X_i=1)=p, \ \Pb(X_i=0)=1-p.
\]
Define the discrete random variable
\[
T = \min\{k \ge 1 : X_k = 1\},
\]
the \emph{time of the first heads}. Then $T$ takes values in $\{1,2,3,\dots\}$ and
\[
\Pb(T=k)
= (1-p)^{k-1}p, \qquad k=1,2,\dots
\]
This is the \emph{geometric distribution}. Its cdf is
\[
F_T(k)
= \Pb(T \le k)
= 1-(1-p)^{k}, \qquad k=1,2,\dots
\]
and
\[
\E[T] = \frac{1}{p}, 
...
\[
\Pb(T=k)
= \frac{1}{2^k}, \qquad k=1,2,\dots.
\]
Where it holds that $\sum_{k=1}^\infty \Pb(T=k) = \sum_{k=1}^\infty \frac{1}{2^k} = 1$.
...
\begin{mdframed}[style=ejemplo, frametitle={\center Gaussian distribution}]
In Figure \ref{fig:gaussian_pdf} we can see the probability density function and the cumulative density function of the one dimensional Gaussian distribution. We will define this distribution in Section~\ref{cap:prob_Gaussian}.
\end{mdframed}

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\textwidth]{img/probability_gaussian_pdf_cdf.pdf}
    \caption{Probability and cumulative density function.}
...
We show the sigmoid function in Figure \ref{fig:sigmoid}. A visualisation of an adjusted logistic regression model was shown in Figure\ref{fig:lr_crickets}.
\end{mdframed}
...
\begin{minipage}{0.4\textwidth}
	\centering
	\includegraphics[width=0.8\textwidth]{img/probability_sigmoid.pdf}
	\captionof{figure}{Visualisation of a sigmoid function.}
	\label{fig:sigmoid}
\end{minipage}
...
\begin{itemize}
	...
	\item The maths are simple: this is key in applications
\end{itemize}
...
\begin{itemize}
	...
	\item Linear transformations of Gaussian vectors are still Gaussian: if $\mathbf{X} \sim \cN(\boldsymbol{\mu},\Sigma)$ and $A$ is a matrix, $A\mathbf{X} + b \sim \cN(A\boldsymbol{\mu}+b, A \Sigma A^\top)$. This enables sampling of multivariate Gaussians with arbitrary mean and covariance.
	\item Central in probabilistic ML: used in multivariate regression, Gaussian processes, PCA, and Bayesian inference.
\end{itemize}

Recall that Figure \ref{fig:lr_crickets} shows a mixture of two two-dimensional Gaussian distributions with different covariance matrices.
...
\begin{mdframed}[style=ejemplo, frametitle={\center Example: $\chi^2$ family of distributinons.}]
A random variable $Z$ is said to distribute according to a $\chi^2$ (denoted $Z \sim \chi^2$) when $Z = \sum^k_{i=1} X^2_i$ for $X\sim\mathcal{N}(0,1)$. We provide a visualisation of the probability density function of $\chi^2$ distributions for various $k$ in Figure \ref{fig:parabola_momentum}.
\end{mdframed}

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\textwidth]{img/probability_xi-squared.pdf}
    \caption{Gaussian distribution and $\chi^2$ distribution for different $k$ parameters.}
    \label{fig:parabola_momentum}
\end{figure}


\subsection{Sampling}
\label{cap:prob_sampling}
...
\end{equation}
Due to the Strong Law of Large Numbers (SLLN), we know that $I_N$ gets \textbf{arbitrarily close} to $I_f$ as $N\to\infty$. This approach to computing expectation is referred to as \textbf{Monte Carlo approximation}.

A relevant question is how to generate the samples needed to approximate $P_N$, this is known as \textbf{sampling}. When sampling directly from a distribution is not possible, there are three main approaches to sampling as follows:
...
In some settings involving more than one random variable, sampling is naturally performed by
\textbf{sampling in stages}.   In this case, a random variable $X$ is generated by first sampling an auxiliary variable
$Z$, and then sampling $X$ conditional on $Z$.  
This is called \textbf{hierarchical sampling} (or sampling from a hierarchical
model).

Formally, if $Z \sim p(z)$ and $X \mid Z=z \sim p(x \mid z)$ is possible to sample, then sampling from the \textbf{marginal} distribution $p(x)$ is obtained by:
\[
z \sim p(z), \qquad x \sim p(x \mid z).
\]


\begin{mdframed}[style=ejemplo, frametitle={\center Example: Gaussian mixture.}]
	The Gaussian mixture model (GMM) is a random variable with a pdf given by a mixture of Gaussian components. For mixture weights $\pi_k \ge 0$, with $\sum_{k=1}^K \pi_k = 1$, and component
	densities $X \mid Z=k \sim \mathcal{N}(\mu_k,\Sigma_k)$, the marginal density of $X$ is
	\[
	p(x)
	= \sum_{k=1}^K \pi_k \,
	...
	\[
	\Pb(Z=1)=\pi,\qquad \Pb(Z=2)=1-\pi,
	\]
	and then as sample of $X$ can be obtained via: 
	\[
	X \mid (Z=1) \sim \mathcal{N}(\mu_1,\sigma_1^2),
	\qquad
	X \mid (Z=2) \sim \mathcal{N}(\mu_2,\sigma_2^2).
	\]

	\textbf{The extension to arbitrary $K>2$ is left as an exercise.} Notice that in this case, the component index $Z$ is given by a Categorical distribution with parameters $\pi_k \ge 0$, with $\sum_{k=1}^K \pi_k = 1$.
\end{mdframed}
